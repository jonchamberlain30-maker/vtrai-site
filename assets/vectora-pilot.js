(() => {
  ...
  $('feedback').addEventListener('submit', async event => {
    ...
    try {
      await send({id: record.id, role: $('role').value, outcome: new FormData($('feedback')).get('outcome'), feedback: $('details').value, next_step: $('next').value, consent: $('consent').checked, website: $('website').value, source, device: matchMedia('(max-width: 600px)').matches ? 'mobile' : 'desktop'});
      record.sent = true;
      remember();
      displaySent();
